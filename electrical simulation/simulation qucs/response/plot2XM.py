...
def parse_ngspice_file(filepath: str) -> tuple[dict, int]:
    """Lee el archivo .dat.ngspice y extrae time y variables dependientes (Universal)."""
    ...
    if not time_match:
        raise ValueError("No se encontró el bloque <indep time N>. Verifica el formato del archivo.")

    n_rows = int(time_match.group(1))
    time_data = [float(x) for x in time_match.group(2).split()]
    print(f"   ✅ Variable independiente 'time' extraída con {n_rows} puntos.")

    # =================================================================
    # NUEVA LÓGICA DE EXTRACCIÓN UNIVERSAL
    # =================================================================
    # Atrapa cualquier cosa entre '<dep ' y ' time>' sin exigir paréntesis
    dep_pattern = re.compile(r'<dep\s+(.*?)\s+time>(.*?)</dep>', re.DOTALL)
    variables_data = {'time': time_data}

    for match in dep_pattern.finditer(content):
        raw_var = match.group(1) # Ej: 'tran.R(Mp01)', 'tran.mp01', 'tran.v(v01)'
        
        # 1. Quitar el prefijo interno de ngspice
        var_name = raw_var.replace('tran.', '') 
        
        # 2. Quitar los paréntesis si los tiene (ej: v(N1)-> N1, R(Mq10)-> Mq10)
        var_name = re.sub(r'^[a-zA-Z]\((.*?)\)$', r'\1', var_name)
        
        # 3. Normalizar memristores: si Qucs escupe 'mp01', lo pasa a 'Mp01'
        # para que coincida perfectamente con tus VARIABLES_INTERES_R
        if len(var_name) >= 3 and var_name[0].lower() == 'm' and var_name[1].lower() in ['p', 'q']:
            var_name = var_name.capitalize() 

        values = [float(x) for x in match.group(2).split()]
        variables_data[var_name] = values
        print(f"   📦 Encontrada: '{var_name}' ({len(values)} puntos)")
...
def construir_dataframe(data_dict: dict, variables_interes: list) -> pl.DataFrame:
    ...
    if no_encontradas:
        print(f"   ⚠️ [ADVERTENCIA] Variables no presentes en el archivo: {no_encontradas}")
    if not disponibles:
        raise ValueError("Ninguna de las variables de interés se encontró en los datos.")
...
def generar_graficas(df: pl.DataFrame, variables_v: list, variables_r: list, output_dir: str = 'IMG', dpi: int = 300):
    ...
    print(f"[3/4] Generando gráficas con escalado SI automático y guardando en '{output_dir}/'...")
    ...
    def graficar_combinada(vars_list, unit_str, label_str, filename,threshold=None):
        ...
        print(f"   🖌️ Graficando variables combinadas de {label_str} en escala lineal pura...")
        ...
        plt.close(fig)
        print(f"   ✅ Guardado: {out_path} | Eje Y en [{unit_str}] puros")
    ...
    def graficar_combinadaLoga(vars_list, unit_str, label_str, filename, use_log=False,threshold=None):
        ...
        print(f"   🖌️ Graficando variables combinadas de {label_str}...")
        ...
        fig.savefig(out_path, dpi=dpi)
        plt.close(fig)
        print(f"   ✅ Guardado: {out_path} | Eje Y: [{prefix_display}{unit_str}]")
    ...
    def graficar_individual(var, unit_str, label_str):
        ...
        print(f"   🖌️ Graficando '{var}' ({label_str})...")
        var_np = df[var].to_numpy()
        ...
        ax.set_title(f'Respuesta Transitoria: {var}')
        ax.grid(True, alpha=0.3, linestyle='--')
        ax.ticklabel_format(axis='both', style='plain', useOffset=False)
...
def main():
    ARCHIVO_ENTRADA = 'Grid3x3.dat.ngspice' # Cambia por tu archivo real
    ...
    print("="*60)
    print("🚀 INICIANDO PROCESAMIENTO NGSPICE/QUCS + ESCALADO SI")
    print("="*60)
    ...
    try:
        ...
        print("\n" + "="*60)
        print("🎉 [COMPLETADO] Gráficas generadas con prefijos SI automáticos")
        print("💡 Se generaron gráficas combinadas separadas para Voltajes y Resistencias.")
        print("="*60)
