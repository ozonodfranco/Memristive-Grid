...
# ==============================================================================
# SECCIÓN 1: NÚCLEO MATEMÁTICO (JIT) - FLUJO (Orden 1, k=5)
# ==============================================================================
@njit(fastmath=True)
...
@njit(parallel=True, fastmath=True)
def hardware_reset_pulse(phi_p, phi_q, Mp, Mq, Mp1, Mp2, Mq1, Mq2, flux_erase_amount, X0, Ron, Roff, kappa,phi_p2,phi_q2):
    M_p, N_p = phi_p.shape
    M_q, N_q = phi_q.shape
    ...
    for i in prange(M_p):
        for j in range(N_p):
            # 1. Aplicar decaimiento al flujo del memristor 1 (Mp1)
            if phi_p[i, j] > 0: phi_p[i, j] = max(0.0, phi_p[i, j] - flux_erase_amount)
            elif phi_p[i, j] < 0: phi_p[i, j] = min(0.0, phi_p[i, j] + flux_erase_amount)
            
            # 2. Aplicar decaimiento al flujo del memristor 2 (Mp2)
            if phi_p2[i, j] > 0: phi_p2[i, j] = max(0.0, phi_p2[i, j] - flux_erase_amount)
            elif phi_p2[i, j] < 0: phi_p2[i, j] = min(0.0, phi_p2[i, j] + flux_erase_amount)
            
            # 3. Recalcular las memristancias usando el flujo correspondiente para cada uno
            mp1_new = calculate_memristance_flux(phi_p[i, j], -1, X0, Ron, Roff, kappa)
            mp2_new = calculate_memristance_flux(phi_p2[i, j], 1, X0, Ron, Roff, kappa)
            
            Mp1[i, j] = mp1_new
            Mp2[i, j] = mp2_new
            Mp[i, j] = mp1_new + mp2_new

    # --- RESET RED HORIZONTAL ---
    for i in prange(M_q):
        for j in range(N_q):
            # 1. Aplicar decaimiento al flujo del memristor 1 (Mq1)
            if phi_q[i, j] > 0: phi_q[i, j] = max(0.0, phi_q[i, j] - flux_erase_amount)
            elif phi_q[i, j] < 0: phi_q[i, j] = min(0.0, phi_q[i, j] + flux_erase_amount)
            
            # 2. Aplicar decaimiento al flujo del memristor 2 (Mq2)
            if phi_q2[i, j] > 0: phi_q2[i, j] = max(0.0, phi_q2[i, j] - flux_erase_amount)
            elif phi_q2[i, j] < 0: phi_q2[i, j] = min(0.0, phi_q2[i, j] + flux_erase_amount)
            
            # 3. Recalcular las memristancias usando el flujo correspondiente para cada uno
            mq1_new = calculate_memristance_flux(phi_q[i, j], -1, X0, Ron, Roff, kappa)
            mq2_new = calculate_memristance_flux(phi_q2[i, j], 1, X0, Ron, Roff, kappa)
            
            Mq1[i, j] = mq1_new
            Mq2[i, j] = mq2_new
            Mq[i, j] = mq1_new + mq2_new

# ==============================================================================
# SECCIÓN 2: CONTROL DE RED DINÁMICA
# ==============================================================================
def init_network_state(M, N, Rinit):
    ...
    return (u_curr, u_next, Mp1, Mp2, Mp, phi_p, Mq1, Mq2, Mq, phi_q,phi_p2,phi_q2)

def process_memristive_frame_dynamic(image_path, output_dir, state, params, flux_erase_amount):
    Rinit, Roff, Ron, Rin, kappa, X0, dt, tn, Mum = params
    u_curr, u_next, Mp1, Mp2, Mp, phi_p, Mq1, Mq2, Mq, phi_q,phi_p2,phi_q2 = state
    ...
    # 1. Reset VLSI (Evita saturación por memoria)
    hardware_reset_pulse(phi_p, phi_q, Mp, Mq, Mp1, Mp2, Mq1, Mq2, flux_erase_amount, X0, Ron, Roff, kappa,phi_p2,phi_q2)
...
if __name__ == "__main__":
    ...
    # Coloca tus videos originales en la carpeta "input_videos"
    ...
    # Tiempo de exposición por frame (dinámico)
    tfin = 0.018 
    dt = 0.0001
    tn = int(tfin / dt)
    ...
    # PARÁMETRO DE RESET VLSI: Ajusta esto si ves "fantasmas" (súbelo) o parpadeos (bájalo)
    flux_erase_amount = 0.0001 # 0.0005
    ...
    for local_video_in in videos_encontrados:
        ...
        # Rutas dentro del espacio de trabajo temporal
        ...
        if files:
            # Leer el primer frame solo para obtener las dimensiones (M, N)
            sample_img = cv2.imread(files[0], cv2.IMREAD_GRAYSCALE)
            M, N = sample_img.shape
            
            print(f"Start memristive grid {N}x{M}...")
            network_state = init_network_state(M, N, Rinit)
            
            # Procesar frame a frame compartiendo el mismo estado
            for f in files:
                process_memristive_frame_dynamic(f, local_output_folder, network_state, params, flux_erase_amount)
